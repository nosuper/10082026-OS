"""What a client still owes when a job is accepted (#153).

Named for the document it fills - biên bản nghiệm thu, the acceptance
record - and deliberately NOT `settlement`, which this codebase already
uses for a different thing: lib/settlement.py settles a producer's cash
float. Two domains, one English word.
...
def collected_bands(payments):
    """What the client has paid, split pre-VAT and VAT.

    Each payment carries the rate it was invoiced at rather than the
    job's current one - a milestone billed at 8% before a rate change
    was collected at 8%, and re-splitting it at today's rate would move
    money between the two bands of a document about the past.

    A payment whose rate nobody recorded refuses: its total still counts,
    but the split cannot be stated, so the two bands become unknown
    while the total stays true. That is the honest shape - we know what
    arrived, not how it divides.
    """
    total = Decimal(0)
    pre_vat = Decimal(0)
    vat = Decimal(0)
    ...
    for payment in payments or []:
        ...
        if amount is None:
            return {"pre_vat": None, "vat": None, "total": None}
        total += amount
        rate = _amount(payment.get("vat_pct"))
        if rate is None:
            splittable = False
            continue
        ...
        vat += amount - net
    return {
        "pre_vat": pre_vat if splittable else None,
        "vat": vat if splittable else None,
        "total": total,
    }
