...
**Purpose:** decide whether [#20 (T3.1: card collaboration)](https://github.com/nosuper/10082026-OS/issues/20) and [#21 (T3.2: table view + fields)](https://github.com/nosuper/10082026-OS/issues/21) are complete and can be **closed** - [PR #23](https://github.com/nosuper/10082026-OS/pull/23) is already merged to `main` - or whether fix tickets need filing. Two answers in here are **blocking**: the project-type list and who may create tags.
...
> **Note on the test site:** it now runs **merged `main`** (verified identical tree, 2026-08-10) - T5 and T3.1+T3.2 together, exactly what production will get. No branch-flipping needed.
...
> List accepted as seeded. Management rule: **founder and admin accounts may add, edit, and remove** project-type values. *(Answered 2026-08-10.)*
...
> Keep it open - both seats may create tags.

### Sources - Website / Referral / Zalo / Expo. Does that cover where last quarter's deals actually came from, or is something missing?

> List accepted. Management rule **changed**: **founder, admin, and producer** may all add, edit, and remove source values (supersedes the founder-only guard built in T3.2).
...
> It's okay - refine the UX/UI in a later phase.
...
> Wanted: **@mention, edit own comment, delete own comment**. Later: **email integration** - attach an email loop to a deal and send email from the card.

## The table view (T3.2)

### Toggle Board → Table. Is the same set of deals there, and does clicking a row open the same card dialog?

> Works, plus change requests: allow **adding a deal as a new line** and **inline editing** of cells for speed; **clicking the deal name** (not the whole row) opens the card dialog.
...
> Current set fine, but make columns **user-configurable - add/remove columns** from the table.
...
> Should **remember the user's choice**.
...
> Prefer **`250,000,000 ₫`** - comma grouping with the ₫ sign - **everywhere in the app**: display, input fields, and outputs.
...
> Couldn't find where to add a source at all - no discoverable entry point in the SPA. (Moot as a guard-rail check: the decision above now allows producers to manage sources too. Superseded by the Settings-screen request below.)
...
> Need a **file manager**: a place to view all files across deals and manage them, with **sharing** later.

### Comments are plain text - no formatting, no images pasted inline. Enough for "khách muốn quay trước Tết", or do you need more?

> More - **inline images in comments** especially.
...
> Not discoverable - build a **Settings screen in the SPA** for the admin and founder roles to manage these lists.
